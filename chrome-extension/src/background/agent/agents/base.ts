...
export abstract class BaseAgent<T extends z.ZodType, M = unknown> {
  ...
  private setWithStructuredOutput(): boolean {
    ...
    // Groq gpt-oss models reject forced tool calls (Nanobrowser sends a tool
    // named 'json' via tool_choice, but Groq returns 400
    // "attempted to call tool 'json' which was not in request.tools").
    // Fall back to manual JSON extraction, which works for gpt-oss-20b.
    if (this.modelName.includes('gpt-oss')) {
      logger.debug(`[${this.modelName}] gpt-oss doesn't support forced tool calls, using manual JSON extraction`);
      return false;
    }
